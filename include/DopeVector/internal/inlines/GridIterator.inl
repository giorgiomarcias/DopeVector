...
namespace dope {
	...
	namespace internal {
		...
		template < typename T, SizeType Dimension >
		inline GridIterator<T, Dimension>::GridIterator(pointer ptr, const IndexD &range, const SizeType i)
			: _ptr(ptr),
			_range(range),
			_i(i)
		{ }
		...
		template < typename T, SizeType Dimension >
		inline GridIterator<T, Dimension>::GridIterator(pointer ptr, const IndexD &range, const IndexD &index)
			: _ptr(ptr),
			_range(range),
